...
## Stage 5. 콘크리트 받아들이기 시험 생성 구현 [completed]
...
체크포인트:
- [x] `concrete_delivery_csi` 전용 registration step을 정의한다
- [x] 현장별 레미콘 송장과 시험 자료 input을 manifest에 연결한다
- [x] 타설 위치, 타설일, 시험일, 업체 정보를 확인하는 화면을 만든다
- [x] 여러 송장 또는 업체가 있는 경우 선택/요약 UI를 둔다
- [x] 생성 완료 시 현장별 받아들이기 시험 `outputExcel`을 결과로 연결한다
- [x] 결과 화면에 최종 Excel 파일명과 시험 요약을 보여 준다
...
체크포인트:
- [x] `concrete_strength_csi` 전용 registration step을 정의한다
- [x] 현장별 7일/28일 강도 자료 input을 manifest에 연결한다
- [x] 시험일, 재령, 타설 위치, 업체별 결과를 구분해 보여 준다
- [x] 7일 자료만 있거나 28일 자료만 있는 현장 상태를 명확히 표시한다
- [x] 생성 완료 시 현장별 압축강도 시험 `outputExcel`을 결과로 연결한다
- [x] 결과 화면에 최종 Excel 파일명과 강도 요약을 보여 준다
