...
| P0 | 공정표 버전 | `src/features/desktop-schedule/state/useDesktopScheduleViewModel.ts` | 버전 선택, 복제본 생성, 비교 열기, 기준 공정표 반영 | `app_action` / `schedule.select_version`, `schedule.create_draft_version`, `schedule.open_version_review`, `schedule.promote_version` |
| P0 | 공정표 편집 | `src/features/desktop-schedule/state/useDesktopScheduleViewModel.ts` | 작업 생성/삭제, 이동/리사이즈 완료, 작업 연결 생성/삭제, 마일스톤 생성/삭제 | `app_action` / `schedule.create_item`, `schedule.delete_selection`, `schedule.move_item`, `schedule.resize_item`, `schedule.create_connection`, `schedule.remove_connection`, `schedule.create_milestone`, `schedule.remove_milestone` |
| P0 | 공정표 update | `src/features/desktop-schedule/state/useDesktopScheduleViewModel.ts` | 공종/세부공종/작업/마일스톤 이름 변경, 색상 변경, 공정 분류 순서 변경 | `app_action` / `schedule.rename_division`, `schedule.rename_work_type`, `schedule.rename_sub_work_type`, `schedule.rename_item`, `schedule.rename_milestone`, `schedule.change_color`, `schedule.reorder_reference` |
...
| P0 | 공정표 보조 기능 | `src/features/desktop-schedule/ui/DesktopSchedulePage.vue`, `src/features/desktop-schedule/state/useDesktopScheduleViewModel.ts` | 작업일보 패널 토글, AI 검증 토글/flag | `app_action` / `schedule.toggle_daily_report_panel`, `schedule.toggle_ai_verification`, `schedule.toggle_ai_flag` |
...
| P0 | AI 에이전트 | `src/features/ai-agent/state/useThreadListViewModel.ts`, `src/features/ai-agent/state/useChatViewModel.ts` | thread 생성/삭제, message 전송, 참여자 초대, 수동 재연결 | `app_action` / `ai_agent.create_thread`, `ai_agent.delete_thread`, `ai_agent.send_message`, `ai_agent.invite_user`, `ai_agent.manual_reconnect` |
...
체크포인트:
- [ ] `useAuthStore.login/logout`에 `success/fail` event를 붙인다
- [ ] `ProjectPicker.selectProject`에 프로젝트 전환 event를 붙인다
- [ ] `DesktopDashboardPage`에 공정률 탭 변경과 TODO 토글 event를 붙인다
- [ ] `useDesktopScheduleViewModel`의 async mutation 함수에 성공/실패 event를 붙인다
- [ ] 공정표 이동/리사이즈처럼 pointer interaction은 `end*Session` 완료 시점에만 event를 보낸다
- [ ] 공정표 이름 변경, 색상 변경, 공정 분류 순서 변경 같은 update event를 붙인다
- [ ] 문서 선택, 업로드 추가, 생성 요청, 분석 성공/실패, 생성 성공/실패, 다운로드 event를 붙인다
- [ ] 작업일보 작성에서 작업 항목 생성/수정/삭제, 사진 추가, 저장 CTA event를 붙인다
- [ ] AI thread 생성/삭제, message 전송, 초대, 수동 재연결 event를 붙인다
- [ ] 기존 admin `trackAction` 호출이 GA adapter를 통해 같은 `app_action`으로 전송되는지 확인한다
- [ ] 실패 event에는 `status_group` 또는 `error_kind`처럼 민감하지 않은 분류만 담는다
...
- 수동 QA 체크리스트
- GA DebugView 확인 기록
- 필요한 경우 analytics adapter 단위 테스트

체크포인트:
- [ ] `npm run build`로 타입과 번들 오류가 없는지 확인한다
- [ ] dev server에서 route 이동과 action payload가 console mirror에 찍히는지 확인한다
- [ ] 운영 임시 확인이 필요하면 브라우저 콘솔에서 `localStorage.setItem("conelp.analytics.debug", "true")` 후 payload mirror를 확인하고, 끝난 뒤 `localStorage.removeItem("conelp.analytics.debug")`를 실행한다
- [ ] 로그인 성공/실패 event가 각각 한 번만 찍히는지 확인한다
- [ ] 프로젝트 전환 후 reload 전 event가 전송되는지 확인한다
- [ ] 공정표에서 복제본 생성, 작업 생성, 엑셀 내보내기, 기준 반영 event를 확인한다
- [ ] 공정표에서 공종/작업/마일스톤 이름 변경과 색상 변경 event를 확인한다
- [ ] 문서 생성 흐름에서 선택 -> 업로드 -> 분석 -> 검토 제출 -> 생성 -> 다운로드 event를 확인한다
- [ ] 작업일보 작성에서 작업 항목 생성/수정/삭제와 사진 추가 event를 확인한다
- [ ] AI 에이전트에서 thread 생성, message 전송, 초대 event를 확인한다
- [ ] event parameter에 파일명, 채팅 본문, 사용자명, 프로젝트명이 없는지 확인한다
- [ ] GA DebugView에서 `app_action`, `route_view`, `app_error`가 들어오는지 확인한다
